...
const kpSchema = z.object({
  ...
  complaint_category: z.enum([
    'property_dispute', 'physical_injury', 'oral_defamation',
    'theft', 'damage_to_property', 'family_dispute',
    'noise_disturbance', 'trespassing', 'others'
...
const KPCaseForm = ({ onSuccess, onCancel }) => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({
    ...
    defaultValues: {
      ...
      complaint_category: 'property_dispute', nature_of_complaint: '',
      is_blotter: false
    }
  });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err.response?.data?.message || 'Error filing the KP Case');
    } finally {
      setLoading(false);
    }
  };
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
               ...
               <select className="form-input" {...register('complaint_category')}>
                  ...
                  <option value="oral_defamation">Oral Defamation</option>
