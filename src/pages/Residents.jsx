...
import { Search, PlusCircle, Filter, Edit, Trash2, Archive, UserCheck, ShieldAlert, RefreshCcw } from 'lucide-react';
...
const Residents = () => {
  ...
  const fetchResidents = async () => {
    ...
    try {
      const res = isArchiveView 
        ? await residentsAPI.getDeleted({ search: searchTerm, sector: filterSector })
        : await residentsAPI.getAll({ search: searchTerm, sector: filterSector });
      if (res.data.success) {
  ...
  }, [searchTerm, filterSector, isArchiveView]);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to soft delete this resident? They will be moved to the archive.')) {
      ...
      try {
        ...
        toast.success("Resident moved to archive");
        fetchResidents();
      } catch (err) {
  ...
  const handleRestore = async (id) => {
    if (window.confirm('Are you sure you want to restore this resident?')) {
      try {
        await residentsAPI.restore(id);
        toast.success("Resident successfully restored");
        fetchResidents();
      } catch (err) {
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          {isArchiveView && canDelete() && (
                            <button 
                              ...
                              title="Restore Resident to Active"
