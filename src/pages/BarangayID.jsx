...
const BarangayID = () => {
  ...
  const handleGenerateID = async () => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('already')) {
        ...
        toast('An active ID already exists for this resident. Fetching existing ID...', { icon: 'ℹ️' });
        try {
        ...
        toast.error(msg || 'Failed to generate ID.');
  ...
  const handleDownloadPDF = async () => {
    if (!resident || !idData) { 
      toast.error('Please generate the ID card first before downloading.'); 
      return; 
    }
    const el = idCardRef.current;
    if (!el) { toast.error('ID card is not rendered yet.'); return; }
